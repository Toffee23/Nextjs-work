...
const setTokens = (accessToken: string, refreshToken: string): void => {
  if (typeof window !== 'undefined') {
    ...
    document.cookie = `access_token=${accessToken}; path=/; max-age=${7 * 24 * 60 * 60}; SameSite=Lax${secure}`;
  }
};
...
// --- In-Flight Refresh Queue Tracking ---
...
if (!baseURL) {
  throw new Error("NEXT_PUBLIC_API_BASE_URL is missing in environment.");
}
...
// --- Request Interceptor: Centralized Auth ---
