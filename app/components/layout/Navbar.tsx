...
const dropdownCategories = [
  ...
  { value: "435-baggy", label: "      Baggy Jeans" },
...
function NavbarComponent() {
  ...
  // Safe nested isolated initialization handler block squashing standard execution lifecycle warnings
  useEffect(() => {
  const handleSyncFetch = async () => {
    // Structural safety guard: don't even talk to the database if user object isn't hydrated
    if (!user) {
      setRecentSearches([]);
      return;
    }
 ...
 // ❌ FIND AND REPLACE THIS CONTAINER BLOCK IN YOUR NAVBAR:
return (
  <div className="relative h-[80px] md:h-[155px] text-left"> 
    <header className={`w-full bg-white font-sans fixed top-0 left-0 z-50 transition-all duration-300 ${isSticky ? 'shadow-md py-0' : ''}`}>
      
      {/* 1. Main Header Bar (ONLY show this if NOT sticky) */}
      {!isSticky ? (
        <div className="py-4 md:py-6 border-b border-gray-100 bg-white">
          <div className="max-w-7xl mx-auto px-4 flex items-center justify-between gap-4 md:gap-8">
            ...
            <div className="hidden md:block flex-1 max-w-2xl relative z-50" ref={searchRef}>
              <form onSubmit={handleSearchSubmit} className="flex w-full items-center border border-slate-200 rounded-md bg-[#f8fafc] focus-within:border-[#149fcd] focus-within:bg-white transition-all shadow-xs">
                <div className="pl-4 text-slate-400"><Search size={18} /></div>
                <input 
                  type="text" 
                  placeholder="Try 'iPhone 15 Pro Max', 'Air Force 1', 'PS5 Slim'..." 
                  value={searchQuery}
                  onChange={e => setSearchQuery(e.target.value)}
                  onFocus={() => setShowRecentPanel(true)}
                  className="flex-1 px-4 py-3 outline-none text-sm text-slate-700 bg-transparent placeholder:text-slate-400 font-medium" 
                />
                <div className="h-5 w-[1px] bg-slate-200 shrink-0" />
                
                <div className="relative flex items-center h-full max-w-[160px]">
                  <select 
                    name="categories"
                    aria-label="Product categories"
                    value={selectedCategory}
                    onChange={e => setSelectedCategory(e.target.value)}
                    className="appearance-none bg-transparent font-sans font-bold text-slate-500 text-xs px-4 py-3 outline-none cursor-pointer pr-8 max-w-full truncate text-ellipsis"
                  >
                ...
                <button type="submit" className="bg-[#149fcd] px-7 py-3 text-white text-sm font-bold rounded-r-md hover:bg-[#118eb8] transition-colors shrink-0 shadow-xs h-full">
                  Search
                </button>
              </form>
            ...
            <div className="hidden md:flex items-center gap-7">
              <Link href="/wishlist" className="flex items-center gap-2 cursor-pointer group text-slate-700 hover:text-[#149fcd] transition-colors select-none">
                <Heart size={20} className="stroke-[2]" />
                <span className="text-xs font-bold tracking-tight">Wishlist</span>
              </Link>

              <div onClick={() => setIsCartOpen(true)} className="flex items-center gap-2 cursor-pointer group text-slate-700 hover:text-[#149fcd] transition-colors select-none relative">
                <div className="relative">
                  <ShoppingBag size={20} className="stroke-[2]" />
                  <span className="absolute -top-2 -right-2 bg-[#149fcd] text-white text-[9px] rounded-full h-4 w-4 flex items-center justify-center font-bold border border-white shadow-2xs">{totalItemCount}</span>
                </div>
                <span className="text-xs font-bold tracking-tight">Cart</span>
              </div>

              {authLoading ? (
                <div className="animate-pulse select-none w-24 h-4 bg-slate-100 rounded-sm" />
              ) : user ? (
                <div className="flex items-center gap-2 border-l pl-6 border-slate-100 group relative cursor-pointer py-1">
                  <div className="relative w-8 h-8 rounded-full bg-[#149fcd] flex items-center justify-center border border-slate-100 overflow-hidden shrink-0 shadow-3xs">
                    {user.avatar_url ? (
                  ...
                  <div className="text-left max-w-[110px]">
                    <p className="text-[10px] text-slate-400 leading-none mb-0.5 select-none">Hello, sign in</p>
                    <div className="flex items-center gap-0.5">
                      <span className="text-xs font-black text-slate-800 truncate block">{user.name}</span>
                ...
                <Link href="/login" className="flex items-center gap-2 border-l pl-6 border-slate-100 group">
                  <User size={20} className="text-slate-700 stroke-[2]" />
                  <div className="text-left leading-none">
                    <p className="text-[10px] text-slate-400 mb-0.5">Hello, sign in</p>
                    <p className="text-xs font-black text-slate-800 hover:text-[#149fcd] transition-colors">Account ▾</p>
                  </div>
                </Link>
              )}
            </div>
      ...
      {/* 2. Secondary Utility Nav Sub-Row */}
      <div className={`hidden md:block transition-all duration-300 ${isSticky ? 'bg-white py-2 shadow-sm border-b border-slate-100' : 'bg-white py-2'}`}>
        <div className="max-w-7xl mx-auto px-6 flex items-center justify-between">
          <div className="flex items-center gap-8">
            
            {/* Sticky Logo (ONLY show this when sticky) */}
            {isSticky && (
              <Link href="/" className="shrink-0 mr-4 block">
            ...
            {/* DYNAMIC CATEGORY PANEL TRIGGER */}
            {!isSticky && (
              <div className="relative z-[40]">
                <button 
                  onClick={() => setIsCategoryOpen(!isCategoryOpen)} 
                  className="bg-[#1a3d46] hover:bg-[#142e35] text-white px-6 py-3 flex items-center gap-8 text-xs font-bold rounded-md justify-between transition-colors min-w-[210px] shadow-xs"
                >
                  <div className="flex items-center gap-2.5">
                    <Menu size={16} className="stroke-[2.5]" />
                    All Categories
                  </div>
                  <ChevronDown size={12} className={`transition-transform duration-200 text-white/60 ${isCategoryOpen ? 'rotate-180' : ''}`} />
                </button>

                {isCategoryOpen && (
                  <div className="absolute top-full left-0 w-full bg-white border border-slate-100 shadow-xl py-1 flex flex-col rounded-md mt-1 animate-in fade-in slide-in-from-top-2 duration-150">
                    {categoriesList.map((cat, idx) => (
                      <Link
                        key={`${cat.name}-${idx}`}
                        href={`/categories/${cat.name.toLowerCase().replace(/ & /g, '-').replace(/ /g, '-')}`}
                        onClick={() => setIsCategoryOpen(false)}
                        className="flex items-center justify-between px-5 py-3 text-xs font-semibold text-slate-600 hover:bg-slate-50 hover:text-[#149fcd] transition-colors border-b border-slate-50 last:border-0"
                      >
                        <div className="flex items-center gap-2.5">
                          <cat.icon size={14} className="text-slate-400 shrink-0" />
                          <span>{cat.name}</span>
                        </div>
                        {cat.hasArrow && <ChevronRight size={12} className="text-slate-400" />}
                      </Link>
                    ))}
                  </div>
                )}
            ...
            {/* NAVIGATION MAIN RAILS */}
            <nav className="flex gap-8 text-xs text-slate-600 font-bold items-center">
              <Link href="/" className="bg-[#e0f2fe] text-[#0284c7] px-4 py-2 rounded-md transition-colors font-black shadow-3xs">Home</Link>
              <Link href="/shop" className="hover:text-[#149fcd] transition-colors font-medium">Shop</Link>
              <span className="flex items-center gap-1 text-slate-600 hover:text-red-500 cursor-pointer transition-colors font-medium">
                <span className="w-1.5 h-1.5 rounded-full bg-red-500 animate-pulse shrink-0" /> Today&apos;s Deals
              </span>
              <Link href="/shop?tag=new-arrivals" className="hover:text-[#149fcd] transition-colors font-medium">New Arrivals</Link>
              <Link href="/vendors" className="hover:text-[#149fcd] transition-colors font-medium">Top Vendors</Link>
              <Link href="/blog" className="hover:text-[#149fcd] transition-colors font-medium">Blog</Link>
              <Link href="/contact" className="hover:text-[#149fcd] transition-colors font-medium">Contact</Link>
              <Link href="/sell" className="text-amber-700 hover:text-amber-800 transition-colors font-bold">Sell on Jummall</Link>
            </nav>

          </div>

          {/* SHIPPING FLAG INSIGNIA */}
          {!isSticky && (
            <div className="hidden lg:flex items-center gap-2 bg-sky-50 text-[#0284c7] text-[11px] font-black px-4 py-2 rounded-full border border-sky-100/40 select-none">
              <span className="uppercase tracking-wider">Free shipping over ₦100,000</span>
            </div>
          )}
          ...
          {user ? (
            <div className="flex items-center gap-3 pt-2">
              <div className="relative w-10 h-10 rounded-full bg-[#149fcd] flex items-center justify-center font-bold text-sm shrink-0 overflow-hidden border border-white/20">
                {user.avatar_url ? <Image src={user.avatar_url} alt="Avatar" fill className="object-cover" unoptimized /> : user.name.charAt(0).toUpperCase()}
              </div>
