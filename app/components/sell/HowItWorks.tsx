  ...
  return (
    <section className="max-w-7xl mx-auto px-4 py-20">
      <div className="mb-12 select-none">
        <h2 className="text-4xl font-black text-[#001F3F] font-montserrat uppercase tracking-tight">
        ...
        {steps.map((step, index) => (
          <div key={index} className="border-b last:border-b-0 border-slate-200">
            <button
              ...
              className="w-full px-8 py-6 flex items-center justify-between text-left transition-colors hover:bg-slate-50/80 group focus:outline-none"
            ...
            >
              ...
              <div className="w-8 h-8 rounded-full border border-slate-200 flex items-center justify-center text-slate-400 group-hover:border-[#149fcd] group-hover:text-[#149fcd] transition-all shrink-0">
                {activeStep === index ? <Minus size={16} className="stroke-[2.5]" /> : <Plus size={16} className="stroke-[2.5]" />}
              </div>
            </button>
            
            {activeStep === index && (
              <div className="px-8 pb-8 animate-in slide-in-from-top-2 duration-200 text-left">
                <p className="text-slate-500 font-medium text-sm leading-relaxed max-w-4xl">
                  {step.content}
                </p>
              </div>
            )}
