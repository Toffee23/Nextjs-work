        ...
        <section>
          ...
          <div className="space-y-3 text-[12px] font-medium">
            ...
            <div className="pl-1 space-y-3">
              {/* Level 1: Kitchen Items Parent Branch */}
              <div className="flex items-center justify-between text-[#149fcd] font-bold">
                <span className="flex items-center gap-1.5">
                  {/* Soft conditional rendering or visual icon could sit here if needed */}
              ...
              {/* Level 2: Expanded Subcategory block branch layout */}
              <div className="pl-4 space-y-3 border-l border-slate-200/60 font-normal text-slate-500 text-[11px] pt-1">
                {kitchenSubcategories.map((item, idx) => (
                  <Link 
                    key={idx} 
                    href={`/categories/kitchen?subcategory=${item.toLowerCase().replace(/ & /g, '-').replace(/\s+/g, '-')}`}
